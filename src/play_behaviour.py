...
class MiRoClient:
    ...
    def miro_in_view(self):
        """
        Detect Miro based off of coordinates (temporary)
        """
        
        # Gets miro positions
        x0, y0 = self.miro_0_pose.x, self.miro_0_pose.y 
        x, y, theta = self.miro_pose.x, self.miro_pose.y, self.miro_pose.theta
        
        angle_to_miro_0 = math.atan2(y0 - y, x0 - x)

        # Normalize angles to [-pi, pi]
        theta = (theta + math.pi) % (2 * math.pi) - math.pi
        angle_to_miro_0 = (angle_to_miro_0 + math.pi) % (2 * math.pi) - math.pi

        # Define angle tolerance (e.g., 10 degrees)
        angle_tolerance = math.radians(10)

        if abs(theta - angle_to_miro_0) < angle_tolerance:
            return True
        else: 
            return False
    ...
    def look_for_miro(self):
        ...
        if self.just_switched:  # Print once
            print("MiRo is looking for a friend...")
    ...
    #         # Store side on which ball last seen
    #         self.last_side = 1
    ...
    def __init__(self):
        ...
        self.sub_caml = rospy.Subscriber(
            miro_topic + "/sensors/caml/compressed",
            CompressedImage,
            self.callback_caml,
            queue_size=1,
            tcp_nodelay=True,
        ...
        self.sub_camr = rospy.Subscriber(
            miro_topic + "/sensors/camr/compressed",
            CompressedImage,
            self.callback_camr,
            queue_size=1,
            tcp_nodelay=True,
        ...
        # Create subscriber for miro location data
        self.miro_pose = rospy.Subscriber(
            miro_topic + "/sensors/body_pose",
            Pose2D,
            self.callback_miro_pose
        )
        # Create subscriber for second miro location data
